...
import com.jawnz.app.config.KafkaProperties;
...
class JawnzappKafkaResourceIT {
    ...
    void setup() {
        ...
        JawnzappKafkaResource kafkaResource = new JawnzappKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jawnzapp-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/jawnzapp-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
